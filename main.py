...
phase_folder = 'phases'
state_file = os.path.join(process_folder, 'state.json')
...
def save_state(phase, iteration, last_similarity_score):
    with open(state_file, 'w') as f:
        json.dump({'phase': phase, 'iteration': iteration, 'last_similarity_score': last_similarity_score}, f)

def load_state():
    if os.path.exists(state_file):
        with open(state_file, 'r') as f:
            return json.load(f)
    return {'phase': 0, 'iteration': 0, 'last_similarity_score': 0}
...
def process_images(input_image_path, output_image_path, object_folder, process_folder, phase_folder, num_phases, iterations):
    ...
    last_similarity_score = state['last_similarity_score']
    ...
    for phase in range(start_phase, num_phases):
        ...
        with ProcessPoolExecutor() as executor:
            for i in range(start_iteration, iterations):
                ...
                save_state(phase, i, last_similarity_score)  # Save state at each iteration
...
process_images(input_image_path, output_image_path, object_folder, process_folder, phase_folder, num_phases=700, iterations=150)
